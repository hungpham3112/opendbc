...
static int get_addr_check_index(const CANPacket_t *msg, RxCheck addr_list[],
                                const int len) {
  int addr = msg->addr;
  ...
  for (int i = 0; i < len; i++) {
    ...
    if (addr_list[i].status.msg_seen) {
      ...
      if ((addr == addr_list[i].msg[idx].addr) &&
          (msg->bus == addr_list[i].msg[idx].bus) &&
...
static bool rx_msg_safety_check(const CANPacket_t *msg,
  ...
  if (index != -1) {
    ...
    if ((safety_hooks->get_checksum != NULL) &&
      ...
      cfg->rx_checks[index].status.valid_checksum =
          cfg->rx_checks[index]
              .msg[cfg->rx_checks[index].status.index]
              .ignore_checksum;
    }
    ...
    if ((safety_hooks->get_counter != NULL) &&
      ...
      cfg->rx_checks[index].status.wrong_counters =
          cfg->rx_checks[index]
                  .msg[cfg->rx_checks[index].status.index]
                  .ignore_counter
              ? 0
    ...
    if ((safety_hooks->get_quality_flag_valid != NULL) &&
      ...
      cfg->rx_checks[index].status.valid_quality_flag =
          safety_hooks->get_quality_flag_valid(msg);
    } else {
      cfg->rx_checks[index].status.valid_quality_flag =
          cfg->rx_checks[index]
              .msg[cfg->rx_checks[index].status.index]
...
bool safety_rx_hook(const CANPacket_t *msg) {
  ...
  bool whitelisted =
      get_addr_check_index(msg, current_safety_config.rx_checks,
  ...
  // used to detect a relay malfunction or control messages from disabled ECUs
  // like the radar
...
static bool tx_msg_safety_check(const CANPacket_t *msg, const CanMsg msg_list[],
                                int len) {
  int addr = msg->addr;
  int length = GET_LEN(msg);
  ...
  for (int i = 0; i < len; i++) {
    if ((addr == msg_list[i].addr) && (msg->bus == msg_list[i].bus) &&
...
bool safety_tx_hook(CANPacket_t *msg) {
  ...
  if ((current_safety_mode == SAFETY_ALLOUTPUT) ||
      ...
      (current_safety_mode == SAFETY_VINFAST)) {
    whitelisted = true;
  }
...
int safety_fwd_hook(int bus_num, int addr) {
  ...
  // Block messages that are being checked for relay malfunctions. Safety modes
  // can opt out of this in the case of selective AEB forwarding
  const int destination_bus = get_fwd_bus(bus_num);
  if (!blocked) {
    for (int i = 0; i < current_safety_config.tx_msgs_len; i++) {
      const CanMsg *m = &current_safety_config.tx_msgs[i];
      if (m->check_relay && !m->disable_static_blocking && (m->addr == addr) &&
          (m->bus == (unsigned int)destination_bus)) {
        blocked = true;
        break;
      }
    }
...
void speed_mismatch_check(const float speed_2) {
  ...
  bool is_invalid_speed =
      ABS(speed_2 - ((float)vehicle_speed.values[0] / VEHICLE_SPEED_FACTOR)) >
      MAX_SPEED_DELTA;
  if (is_invalid_speed) {
    controls_allowed = false;
  }
